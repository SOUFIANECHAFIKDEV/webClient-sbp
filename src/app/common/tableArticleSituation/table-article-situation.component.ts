...
export class TableArticleSituationComponent implements OnChanges {
  ...
  initData(pourcentage) {
    ...
    //prixTTC
    const restTtc = this.devisInfos.total - (situationCumullee.cumulleeTTC + this.calcul.totalAcomptesTTC);
    ...
    //prixHT
    const restHt = this.devisInfos.totalHt - (situationCumullee.cumulleeHt + totalAcomptes.totalHT)
  ...
  getDateToSave(status: StatutFacture, callBack: any): void {
    ...
    callBack({
      ...
      tva: this.calcul.tva,
      retenueGarantie: this.retenueGarantie,
      totalHt: this.calcul.prixHT,
      totalTtc: this.calcul.prixTTC,
