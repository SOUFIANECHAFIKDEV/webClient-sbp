...
export class SelectUserComponent implements OnInit, OnChanges {
  ...
  constructor(private utilisateurService: UtilisateurService, private translate: TranslateService, @Inject(DOCUMENT) private document: any) { }
  ...
  getTechniciens(technicientToDispaly?) {
    ...
    this.utilisateurService.GetAll(this.search, this.page, AppSettings.NBR_ITEM_PER_PAGE, "nom", "desc", UserProfile.technicien).subscribe(res => {
      this.techniciens = res.list;
      this.totalPage = res.totalPages;
      this.removeTmpFromOrigin();


      // if (technicientToDispaly != null) {
      //   technicientToDispaly.forEach(element => {
      //     this.techniciens = this.techniciens.map(T => {
      //       if (T.id != element.id) return T;
      //     });
      //     this.technicienTmp = [];
      //     this.technicienTmp.push(element);
      //   });
      // }
