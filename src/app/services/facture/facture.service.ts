...
import { FactureSituationbodyRequest } from 'app/Models/Entities/Devis';
...
export class FactureService {
  ...
  GetAll(SearchQuery?, PageNumber?, PageSize?, OrderBy?, SortDirection?, Statut?: number[],/*Status?,*/ dateDebut?, dateFin?, idChantier?, idClient?): Observable<FactureListModel> {
    let body = {
  ...
  Get(id): Observable<Facture> {
    return this.http.get<Facture>(AppSettings.API_ENDPOINT + 'Factures/' + id, AppSettings.RequestOptions());
  }
  ...
  Update(id, body): Observable<Facture> {
    return this.http.put<Facture>(AppSettings.API_ENDPOINT + 'Factures/' + id, body, AppSettings.RequestOptions());
  }
  ...
  CreateFactureSituation(bodyRequest: FactureSituationbodyRequest): Observable<Facture> {
